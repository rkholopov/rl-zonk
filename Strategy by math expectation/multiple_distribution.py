...
compact_dist = [[dist[0], 1]]
for i in range(1, len(dist)):
    if dist[i] != compact_dist[-1][0]:
        compact_dist.append([dist[i], 1])
    else:
        compact_dist[-1][1] += 1
...
for k in range(2):
    ...
    for i in range(len(mult_dist)):
        for j in range(len(compact_dist)):
            x = mult_dist[i][0] + compact_dist[j][0]
            cnt = mult_dist[i][1] * compact_dist[j][1]
            if tuple([x, cnt]) in dict_next_mult_dist.keys():
                next_mult_dist[dict_next_mult_dist[tuple([x, cnt])]][1] += cnt
            else:
                dict_next_mult_dist[tuple([x, cnt])] = len(next_mult_dist)
